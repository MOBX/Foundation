...
public class Bytes {
    ...
    public static String toStringBinary(byte[] b, int off, int len) {
        ...
        try {
            ...
            for (int i = 0; i < first.length(); i++) {
                ...
                } else result.append(String.format("\\x%02X", new Object[] { Integer.valueOf(ch) }));
            }
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    /**
     * public static int putInt(byte[] bytes, int offset, int val) { if ((bytes == null) || (bytes.length - offset < 4))
     * { return offset; } for (int i = offset + 3; i > offset; i--) { bytes[i] = (byte) val; val >>>= 8; } bytes[offset]
     * = (byte) val; return offset + 4; }
     **/
    ...
    public static byte[] incrementBytes(byte[] value, long amount) throws IOException {
        ...
        if (val.length < 8) {
            ...
            throw new IllegalArgumentException(
                                               new StringBuilder().append("Increment Bytes - value too big: ").append(val.length).toString());
        }
