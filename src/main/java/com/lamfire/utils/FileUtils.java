...
public class FileUtils {
    ...
    public static String byteCountToDisplaySize(long size) {
        ...
        if (size / 1073741824L > 0L) {
            ...
            if (size / 1048576L > 0L) {
                ...
                if (size / 1024L > 0L) displaySize = String.valueOf(size / 1024L) + " KB";
    ...
    private static void doCopyDirectory(File srcDir, File destDir, boolean preserveFileDate) throws IOException {
        if (destDir.exists()) {
            if (!destDir.isDirectory()) throw new IOException("Destination '" + destDir
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].isDirectory()) doCopyDirectory(files[i], copiedFile, preserveFileDate);
            else doCopyFile(files[i], copiedFile, preserveFileDate);
        }
    }
    ...
    public static void writeLines(File file, String encoding, Collection<String> lines, String lineEnding)
                                                                                                          throws IOException {
        OutputStream out = null;
        try {
            out = openOutputStream(file);
    ...
    public static void forceDeleteOnExit(File file) throws IOException {
        ...
        else file.getAbsoluteFile().deleteOnExit();
    }
