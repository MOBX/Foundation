...
public class PictureMarker {
    ...
    public final static void markImage(File source, File target, int right, int bottom) {
        try {
            ...
            /**
             * //输出到文件流 FileOutputStream out=new FileOutputStream(target); JPEGImageEncoder encoder =
             * JPEGCodec.createJPEGEncoder(out); encoder.encode(image);//近JPEG编码 out.close();
             **/
