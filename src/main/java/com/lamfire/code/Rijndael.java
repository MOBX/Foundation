...
public final class Rijndael {
    ...
    static {
        /*
         * Te0[x] = Se[x].[02, 01, 01, 03]; Te1[x] = Se[x].[03, 02, 01, 01]; Te2[x] = Se[x].[01, 03, 02, 01]; Te3[x] =
         * Se[x].[01, 01, 03, 02]; Td0[x] = Sd[x].[0e, 09, 0d, 0b]; Td1[x] = Sd[x].[0b, 0e, 09, 0d]; Td2[x] = Sd[x].[0d,
         * 0b, 0e, 09]; Td3[x] = Sd[x].[09, 0d, 0b, 0e];
         */
        int ROOT = 0x11B;
    ...
    private void expandKey(byte[] cipherKey) {
        ...
        for (int i = 0, k = 0; i < Nk; i++, k += 4) {
            rek[i] = ((cipherKey[k]) << 24) | ((cipherKey[k + 1] & 0xff) << 16) | ((cipherKey[k + 2] & 0xff) << 8)
                     | ((cipherKey[k + 3] & 0xff));
        }
        for (int i = Nk, n = 0; i < Nw; i++, n--) {
            ...
            if (n == 0) {
                n = Nk;
                temp = ((Se[(temp >>> 16) & 0xff]) << 24) | ((Se[(temp >>> 8) & 0xff] & 0xff) << 16)
                       | ((Se[(temp) & 0xff] & 0xff) << 8) | ((Se[(temp >>> 24)] & 0xff));
                temp ^= rcon[r++];
            } else if (Nk == 8 && n == 4) {
                temp = ((Se[(temp >>> 24)]) << 24) | ((Se[(temp >>> 16) & 0xff] & 0xff) << 16)
                       | ((Se[(temp >>> 8) & 0xff] & 0xff) << 8) | ((Se[(temp) & 0xff] & 0xff));
            }
            rek[i] = rek[i - Nk] ^ temp;
        }
        temp = 0;
    ...
    /*
     * Faster implementation of the key expansion (only worthwhile in Rijndael is used in a hashing function mode).
     */
    /*
     * private void expandKey(byte[] cipherKey) { int keyOffset = 0; int i = 0; int temp; rek[0] = (cipherKey[ 0] ) <<
     * 24 | (cipherKey[ 1] & 0xff) << 16 | (cipherKey[ 2] & 0xff) << 8 | (cipherKey[ 3] & 0xff); rek[1] = (cipherKey[ 4]
     * ) << 24 | (cipherKey[ 5] & 0xff) << 16 | (cipherKey[ 6] & 0xff) << 8 | (cipherKey[ 7] & 0xff); rek[2] =
     * (cipherKey[ 8] ) << 24 | (cipherKey[ 9] & 0xff) << 16 | (cipherKey[10] & 0xff) << 8 | (cipherKey[11] & 0xff);
     * rek[3] = (cipherKey[12] ) << 24 | (cipherKey[13] & 0xff) << 16 | (cipherKey[14] & 0xff) << 8 | (cipherKey[15] &
     * 0xff); if (Nk == 4) { for (;;) { temp = rek[keyOffset + 3]; rek[keyOffset + 4] = rek[keyOffset] ^ ((Se[(temp >>>
     * 16) & 0xff] ) << 24) ^ ((Se[(temp >>> 8) & 0xff] & 0xff) << 16) ^ ((Se[(temp ) & 0xff] & 0xff) << 8) ^ ((Se[(temp
     * >>> 24) ] & 0xff) ) ^ rcon[i]; rek[keyOffset + 5] = rek[keyOffset + 1] ^ rek[keyOffset + 4]; rek[keyOffset + 6] =
     * rek[keyOffset + 2] ^ rek[keyOffset + 5]; rek[keyOffset + 7] = rek[keyOffset + 3] ^ rek[keyOffset + 6]; if (++i ==
     * 10) { return; } keyOffset += 4; } } rek[keyOffset + 4] = (cipherKey[16] ) << 24 | (cipherKey[17] & 0xff) << 16 |
     * (cipherKey[18] & 0xff) << 8 | (cipherKey[19] & 0xff); rek[keyOffset + 5] = (cipherKey[20] ) << 24 |
     * (cipherKey[21] & 0xff) << 16 | (cipherKey[22] & 0xff) << 8 | (cipherKey[23] & 0xff); if (Nk == 6) { for (;;) {
     * temp = rek[keyOffset + 5]; rek[keyOffset + 6] = rek[keyOffset] ^ ((Se[(temp >>> 16) & 0xff] ) << 24) ^ ((Se[(temp
     * >>> 8) & 0xff] & 0xff) << 16) ^ ((Se[(temp ) & 0xff] & 0xff) << 8) ^ ((Se[(temp >>> 24) ] & 0xff) ) ^ rcon[i];
     * rek[keyOffset + 7] = rek[keyOffset + 1] ^ rek[keyOffset + 6]; rek[keyOffset + 8] = rek[keyOffset + 2] ^
     * rek[keyOffset + 7]; rek[keyOffset + 9] = rek[keyOffset + 3] ^ rek[keyOffset + 8]; if (++i == 8) { return; }
     * rek[keyOffset + 10] = rek[keyOffset + 4] ^ rek[keyOffset + 9]; rek[keyOffset + 11] = rek[keyOffset + 5] ^
     * rek[keyOffset + 10]; keyOffset += 6; } } rek[keyOffset + 6] = (cipherKey[24] ) << 24 | (cipherKey[25] & 0xff) <<
     * 16 | (cipherKey[26] & 0xff) << 8 | (cipherKey[27] & 0xff); rek[keyOffset + 7] = (cipherKey[28] ) << 24 |
     * (cipherKey[29] & 0xff) << 16 | (cipherKey[30] & 0xff) << 8 | (cipherKey[31] & 0xff); if (Nk == 8) { for (;;) {
     * temp = rek[keyOffset + 7]; rek[keyOffset + 8] = rek[keyOffset] ^ ((Se[(temp >>> 16) & 0xff] ) << 24) ^ ((Se[(temp
     * >>> 8) & 0xff] & 0xff) << 16) ^ ((Se[(temp ) & 0xff] & 0xff) << 8) ^ ((Se[(temp >>> 24) ] & 0xff) ) ^ rcon[i];
     * rek[keyOffset + 9] = rek[keyOffset + 1] ^ rek[keyOffset + 8]; rek[keyOffset + 10] = rek[keyOffset + 2] ^
     * rek[keyOffset + 9]; rek[keyOffset + 11] = rek[keyOffset + 3] ^ rek[keyOffset + 10]; if (++i == 7) { return; }
     * temp = rek[keyOffset + 11]; rek[keyOffset + 12] = rek[keyOffset + 4] ^ ((Se[(temp >>> 24) ] ) << 24) ^ ((Se[(temp
     * >>> 16) & 0xff] & 0xff) << 16) ^ ((Se[(temp >>> 8) & 0xff] & 0xff) << 8) ^ ((Se[(temp ) & 0xff] & 0xff));
     * rek[keyOffset + 13] = rek[keyOffset + 5] ^ rek[keyOffset + 12]; rek[keyOffset + 14] = rek[keyOffset + 6] ^
     * rek[keyOffset + 13]; rek[keyOffset + 15] = rek[keyOffset + 7] ^ rek[keyOffset + 14]; keyOffset += 8; } } }
     */
    ...
    private void invertKey() {
        ...
        for (int r = 1; r < Nr; r++) {
            w = rek[e];
            rdk[d] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff] ^ Td2[Se[(w >>> 8) & 0xff] & 0xff]
                     ^ Td3[Se[(w) & 0xff] & 0xff];
            w = rek[e + 1];
            rdk[d + 1] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff]
                         ^ Td2[Se[(w >>> 8) & 0xff] & 0xff] ^ Td3[Se[(w) & 0xff] & 0xff];
            w = rek[e + 2];
            rdk[d + 2] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff]
                         ^ Td2[Se[(w >>> 8) & 0xff] & 0xff] ^ Td3[Se[(w) & 0xff] & 0xff];
            w = rek[e + 3];
            rdk[d + 3] = Td0[Se[(w >>> 24)] & 0xff] ^ Td1[Se[(w >>> 16) & 0xff] & 0xff]
                         ^ Td2[Se[(w >>> 8) & 0xff] & 0xff] ^ Td3[Se[(w) & 0xff] & 0xff];
            d += 4;
            e -= 4;
        }
    ...
    public void makeKey(byte[] cipherKey, int keyBits, int direction) throws RuntimeException {
        ...
        if ((direction & DIR_BOTH) != 0) {
            expandKey(cipherKey);
            /*
             * for (int r = 0; r <= Nr; r++) { System.out.print("RK" + r + "="); for (int i = 0; i < 4; i++) { int w =
             * rek[4*r + i]; System.out.print(" " + Integer.toHexString(w)); } System.out.println(); }
             */
    ...
    public void encrypt(byte[] pt, byte[] ct) {
        ...
        for (int r = 1; r < Nr; r++) {
            ...
            int a1 = Te0[(t1 >>> 24)] ^ Te1[(t2 >>> 16) & 0xff] ^ Te2[(t3 >>> 8) & 0xff] ^ Te3[(t0) & 0xff]
                     ^ rek[k + 1];
            int a2 = Te0[(t2 >>> 24)] ^ Te1[(t3 >>> 16) & 0xff] ^ Te2[(t0 >>> 8) & 0xff] ^ Te3[(t1) & 0xff]
                     ^ rek[k + 2];
            int a3 = Te0[(t3 >>> 24)] ^ Te1[(t0 >>> 16) & 0xff] ^ Te2[(t1 >>> 8) & 0xff] ^ Te3[(t2) & 0xff]
                     ^ rek[k + 3];
            t0 = a0;
            t1 = a1;
            t2 = a2;
    ...
    public void decrypt(byte[] ct, byte[] pt) {
        ...
        for (int r = 1; r < Nr; r++) {
            ...
            int a1 = Td0[(t1 >>> 24)] ^ Td1[(t0 >>> 16) & 0xff] ^ Td2[(t3 >>> 8) & 0xff] ^ Td3[(t2) & 0xff]
                     ^ rdk[k + 1];
            int a2 = Td0[(t2 >>> 24)] ^ Td1[(t1 >>> 16) & 0xff] ^ Td2[(t0 >>> 8) & 0xff] ^ Td3[(t3) & 0xff]
                     ^ rdk[k + 2];
            int a3 = Td0[(t3 >>> 24)] ^ Td1[(t2 >>> 16) & 0xff] ^ Td2[(t1 >>> 8) & 0xff] ^ Td3[(t0) & 0xff]
                     ^ rdk[k + 3];
            t0 = a0;
            t1 = a1;
            t2 = a2;
