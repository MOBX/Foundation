...
public final class SerializeWriter extends Writer {
    ...
    public void writeInt(int i) {
        ...
        int size = (i < 0) ? CharUtils.stringSize(-i) + 1 : CharUtils.stringSize(i);
        ...
        CharUtils.getChars(i, newcount, buf);
    ...
    public void writeShortArray(short[] array) throws IOException {
        ...
        for (int i = 0; i < array.length; ++i) {
            ...
            short val = array[i];
            int size = CharUtils.stringSize(val);
            sizeArray[i] = size;
            totalSize += size;
        }
        ...
        for (int i = 0; i < array.length; ++i) {
            ...
            CharUtils.getChars(val, currentSize, buf);
        }
        buf[currentSize] = ']';
    ...
    public void writeIntArray(int[] array) {
        ...
        for (int i = 0; i < array.length; ++i) {
            ...
            if (val == Integer.MIN_VALUE) {
                ...
                size = (val < 0) ? CharUtils.stringSize(-val) + 1 : CharUtils.stringSize(val);
            }
            sizeArray[i] = size;
            totalSize += size;
        }
    ...
    public void writeIntAndChar(int i, char c) {
        ...
        CharUtils.getChars(i, newcount0, buf);
        buf[newcount0] = c;
    ...
    private void writeStringWithDoubleQuote(String text) {
        ...
        if (specialCount == 1) {
            ...
            buf[++lastSpecialIndex] = CharTypes.replaceChars[(int) lastSpecial];
            ...
            for (int i = lastSpecialIndex - 2; i >= start; --i) {
                ...
                if (ch == '\b' || ch == '\n' || ch == '\r' || ch == '\f' || ch == '\\' || ch == '"' //
                    ...
                    buf[i] = '\\';
                    buf[i + 1] = CharTypes.replaceChars[(int) ch];
                    end++;
                }
            }
        }
    ...
    public void writeKeyWithDoubleQuote(String text) {
        ...
        for (int i = start; i < end; ++i) {
            ...
            if (ch < specicalFlags_doubleQuotes.length && specicalFlags_doubleQuotes[ch] //
                ...
                System.arraycopy(buf, i + 1, buf, i + 2, end - i - 1);
                buf[i] = '\\';
                buf[++i] = CharTypes.replaceChars[(int) ch];
                end++;
            }
        }
